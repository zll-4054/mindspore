...
class DLTensor(ctypes.Structure):
    ...
    def __array_interface__(self):
        ...
        return dict(version=3, shape=shape, strides=strides, data=(self.data, True), offset=self.byte_offset,
                    typestr=typestr)

    def __str__(self):
        return 'dtype={dtype}, ndim={ndim}, shape={shape}, strides={strides}, byte_offset={byte_offset}'.format(
            dtype=self.dtype, ndim=self.ndim, shape=tuple(self.shape[i] for i in range(self.ndim)),
            strides=tuple(self.strides[i] for i in range(self.ndim)), byte_offset=self.byte_offset)
...
def from_dlpack(tensor)
    return mindspore.Tensor(ctypes.byref(dl_managed_tensor), b'dltensor', None)

## dlpack to numpy.array
def numpy_from_dlpack(pycapsule):
    dl_managed_tensor = ctypes.cast(PyCapsule_GetPointer(pycapsule, b'dltensor'),
    ...
    wrapped = type('', (), dict(__array_interface__=dl_managed_tensor.dl_tensor.__array_interface__,
                                __del__=lambda self: dl_managed_tensor.deleter(ctypes.byref(dl_managed_tensor))))()
